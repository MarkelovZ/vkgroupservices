...
if(isset($_POST['chatnamechange'])){
    ...
    if(empty($_POST['peed_id'])){echo'Ошибка: Эй. ID Беседы - ОБЯЗАТЕЛЬНО!! :D <br>';}
	if(empty($_POST['count'])){echo'Ошибка: Минимум 1 в кол-ве сообщений <br>';}
    else{
        $query = file_get_contents("https://api.vk.com/method/messages.getHistory?peer_id=".urlencode($_POST['peer_id'])."&count=".urlencode($_POST['count'])."&v=5.80&access_token=".urlencode($_POST['token']));
...
<meta charset="UTF-8">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<center><h1>Получить историю сообщений (будет сложна!)</h1><br>
<form method="post" action="#">
    <input type="text" name="token" placeholder="Токен группы">
    <br><br>
    <input type="text" name="peer_id" placeholder="ID беседы">
    <br><br><br><br>
    <input type="text" name="count" placeholder="кол-во сообщений"><br>
    Вверху сайта вылезет огромная надпись. Там где "id" - это ид сообщения.
    <br><br>
